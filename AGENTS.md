...
- `{type}/{description}` 形式（feat/, fix/, refactor/, docs/, test/, chore/）
- mainへの直接コミットはGitHub公開後に禁止（PR経由のみ）
...
### テスト規律: TDD (Red-Green-Refactor)

1. **RED**: テストを先に書いて失敗を確認する
2. **GREEN**: 最小限のコードでパスさせる
3. **REFACTOR**: テストが緑のまま整理する

- パーサーのテスト: サンプルgit出力文字列で単体テスト
- ツールのテスト: 一時gitリポジトリを作成して統合テスト
- テストのアサーションを緩めて失敗を回避しない
