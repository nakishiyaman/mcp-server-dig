...
最終更新: 2026-03-13 (npm v0.23.0公開済み — ROADMAP v0.24.0の内容を含む)

> **バージョン番号について**: このROADMAPのセクション番号は開発コードネームであり、
> release-pleaseが決定するnpmバージョンとは一致しない場合がある。
> release-pleaseはConventional Commitsに基づいてバージョンを自動決定するため、
> 複数の開発フェーズが1つのリリースにまとめられることがある。
>
> 既知のずれ:
> - ROADMAP v0.4.1 → npm v0.5.0（release-pleaseがminor bumpと判定）
> - ROADMAP v0.24.0 → npm v0.23.0に含まれてリリース済み
>
> **今後の方針**: 次の開発サイクルからはnpmの実バージョン（v0.24.0）をコードネームとして使用する。
