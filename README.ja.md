...
全41ツールが、大規模リポジトリ向けにオプションの `timeout_ms` パラメータ（デフォルト: 30000ms、最大: 300000ms）を受け付けます。

### 出力フォーマット

全41ツールがオプションの `output_format` パラメータを受け付けます:
- `"text"`（デフォルト）— 人間が読みやすいテキスト形式
- `"json"` — プログラムからの利用に適した構造化JSON形式

### Tool Annotations

全41ツールにMCP Tool Annotations（`readOnlyHint: true`, `openWorldHint: false`）を宣言。digの全ツールが読み取り専用のgit分析操作であることをクライアントに通知します。
