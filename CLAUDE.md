...
| スキーマ | zod |
| テスト | vitest + @fast-check/vitest (プロパティベーステスト) |
| ビルド | tsc → build/ |
...
セッションはいつクラッシュしても不思議ではない。以下の4つを常時実行し、会話消失の被害を最小化する。

1. **計画・意思決定は即ファイルに書き出す** — 議論結果・方針・候補リストは会話内に留めず、`docs/` や ROADMAP に Write する。合意前でも「検討中」として書く
2. **こまめに `wip:` コミット** — 作業途中でもこまめにコミットして保存する。完了後にsquashやamendで整理すればよい
3. **`/handoff` でセッション記録** — セッション終了前に HANDOFF.md に進捗を記録する（ただしクラッシュには間に合わないため、1と2が主防御）
4. **重要な意思決定はメモリに保存** — プロジェクト方針・設計判断・ユーザーフィードバックはメモリファイルに書く。次回セッションで自動復元される
...
- 現在のステータス: **v0.41.0 開発中**（54ツール: データ取得38 + 組み合わせ分析14 + ワークフロー統合2, Prompts 18, Resources 2, Tool Annotations・MCP Logging・Streamable HTTP・Completions対応、server.registerTool()移行済み、TypeScript 5.9、TransportHandle導入、プロパティベーステスト導入）
